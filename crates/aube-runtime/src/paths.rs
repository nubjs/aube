...
/// `cache_dir`: the active embedder's `cache_namespace` (standalone aube →
/// `"aube"`, nub → `"nub/pm"`) under `$XDG_CACHE_HOME` (or `%LOCALAPPDATA%` /
/// `~/.cache`). Uses the namespace const rather than a literal so a host
/// embedder never gets an `aube`-named path written into its cache.
