...
/// Whether the vendor attribution (`by jdx.dev`) may render into the
/// install banner. The vendor tag is aube's *own* credit, so it shows
/// only when the binary is running as standalone aube — i.e. the active
/// embedder is aube's own profile. An embedder (e.g. a host that
/// vendors the engine under its own name) suppresses it: surfacing
/// aube's vendor brand in the host's install output would leak the
/// engine's identity into a brand-policed user-facing surface, which is
/// exactly what the host's brand boundary forbids. Keyed on the
/// embedder's `name` matching standalone aube's, because `embedder()`
/// returns a copied profile (no stable pointer to compare against
/// `AUBE`). Standalone aube keeps `by jdx.dev`; every other embedder
/// gets `display_name VERSION` only.
