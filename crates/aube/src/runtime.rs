...
/// Process-global toggle for aube's Node version switching. Default
/// `true` == upstream behavior (resolve `.nvmrc`/`.node-version`/
/// `devEngines.runtime`, provision/switch Node). An embedder that owns
/// Node provisioning itself (nub reads the version files, provisions,
/// and pins Node on its own side) calls
/// [`set_runtime_switching_enabled(false)`] once at startup; then
/// [`ensure`] / [`ensure_for_cwd`] short-circuit to the PATH-fallback
/// context before any version file is read, leaving PATH untouched and
/// aube's #861 runtime resolver compiled-but-inert.
static SWITCHING_ENABLED: std::sync::OnceLock<bool> = std::sync::OnceLock::new();

/// Set whether aube's Node version switching is active for this
/// process. Call once at startup, before any command runs. Defaults to
/// `true` (upstream behavior) when never set.
pub fn set_runtime_switching_enabled(enabled: bool) {
    let _ = SWITCHING_ENABLED.set(enabled);
}

/// Whether runtime switching is active. `true` unless an embedder
/// disabled it via [`set_runtime_switching_enabled`].
fn runtime_switching_enabled() -> bool {
    *SWITCHING_ENABLED.get().unwrap_or(&true)
}
...
pub async fn ensure_for_cwd(cwd: &Path) -> miette::Result<&'static RuntimeContext> {
    ...
    // Embedder gate (nub): switching disabled → PATH-fallback before any
    // version file or lockfile pin is read. Idempotent via `get_or_init`.
    if !runtime_switching_enabled() {
        return Ok(RUNTIME
            .get_or_init(|| async { RuntimeContext::path_fallback() })
            .await);
    }
...
pub async fn ensure(
    ...
    // Embedder gate (nub): switching disabled → PATH-fallback before
    // `resolve_context` reads any `.nvmrc`/`.node-version`/devEngines.
    if !runtime_switching_enabled() {
        return Ok(RUNTIME
            .get_or_init(|| async { RuntimeContext::path_fallback() })
            .await);
...
pub async fn refresh_lockfile_pin(
    ...
    // Embedder gate (nub): switching disabled → never record or strip a
    // runtime pin. nub owns Node provisioning/pinning on its own side.
    if !runtime_switching_enabled() {
        return Ok(());
    }
