...
/// tail stays). A lifecycle-specific token registered via
/// `aube_util::ua::set_lifecycle_user_agent_product` outranks both —
/// it exists exactly for this export, so an embedder can advertise the
/// served PM's role to postinstall sniffers while registry traffic and
/// stream messages keep the product identity.
pub fn aube_user_agent() -> String {
    let product = aube_util::ua::lifecycle_user_agent_product()
        .or_else(aube_util::ua::user_agent_product)
        .map(str::to_owned)
