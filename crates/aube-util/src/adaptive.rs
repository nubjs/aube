 ...
 * `~/Library/Caches`, or `%LOCALAPPDATA%`. The cache namespace is the
 * active embedder's `cache_namespace` (standalone aube → `"aube"`, nub →
 * `"nub/pm"`), not a hardcoded `"aube"`, so a host embedder never gets an
 * `aube`-named state file written into its cache dir.
 */
